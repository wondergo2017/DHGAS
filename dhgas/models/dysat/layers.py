...
class GATLayer(GATConv):
    def __init__(self, input_dim, output_dim, n_heads, attn_drop, ffd_drop, residual):
        super(GATLayer, self).__init__(
            input_dim, output_dim, n_heads, dropout=attn_drop, concat=False, bias=False
        )
    ...
    def forward(self, graph):
        ...
        # return super().forward(x=org_x,edge_index=edge_index,edge_attr=edge_weight,)
        out = super().forward(x=org_x, edge_index=edge_index)
        if self.residual:
            out = out + self.lin_residual(org_x)
        return out


class StructuralAttentionLayer2(nn.Module):
    def __init__(self, input_dim, output_dim, n_heads, attn_drop, ffd_drop, residual):
        super(StructuralAttentionLayer2, self).__init__()
        self.out_dim = output_dim // n_heads
        self.n_heads = n_heads
        self.act = nn.ELU()
    ...
    def forward(self, x, graph):
        org_x = x  # torch.Size([41521, 32])
        edge_index = graph.edge_index
        # edge_weight = graph.edge_weight.reshape(-1, 1)
        H, C = self.n_heads, self.out_dim
        x = self.lin(org_x).view(
            -1, H, C
        )  # [N, heads, out_dim] torch.Size([41521, 16, 4])
        # x = self.lin(graph.x).view(-1, H, C) # [N, heads, out_dim]torch.Size([2, 3408])
        # attention
        ...
        coefficients = softmax(
            alpha, edge_index[1]
        )  # [num_edges, heads]torch.Size([3408, 16])

        # dropout
        if self.training:
            coefficients = self.attn_drop(coefficients)  # torch.Size([3408, 16])
            x = self.ffd_drop(x)  # torch.Size([41521, 16, 4])
        x_j = x[edge_index[0]]  # [num_edges, heads, out_dim]torch.Size([3408, 16, 4])

        # output
        out = self.act(
            scatter(
                x_j * coefficients[:, :, None],
                edge_index[1],
                dim=0,
                reduce="sum",
                dim_size=x.shape[0],
            )
        )  # torch.Size([41517, 16, 4])
...
class TemporalAttentionLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        q_ = torch.cat(
            torch.split(q, split_size_or_sections=split_size, dim=2), dim=0
        )  # [hN, T, F/h]
        k_ = torch.cat(
            torch.split(k, split_size_or_sections=split_size, dim=2), dim=0
        )  # [hN, T, F/h]
        v_ = torch.cat(
            torch.split(v, split_size_or_sections=split_size, dim=2), dim=0
        )  # [hN, T, F/h]
...
class WindowAttentionLayer(nn.Module):
    def __init__(
        self, input_dim=64, n_heads=8, num_time_steps=10, attn_drop=0.5, residual=True
    ):
    ...
    def forward(self, input_q, inputs, mask):
        """In:  attn_outputs (of StructuralAttentionLayer at each snapshot):= [N, T, F]"""
        # 1: Add position embeddings to input
        ...
        # 2: Query, Key based multi-head self attention.
        q = torch.tensordot(
            input_q, self.Q_embedding_weights, dims=([2], [0])
        )  # [N, T, F]
        k = torch.tensordot(
            temporal_inputs, self.K_embedding_weights, dims=([2], [0])
        )  # [N, T2, F]
        ...
        outputs = torch.matmul(q_, k_.permute(0, 2, 1))  # [hN, T, T2]
        outputs = outputs / (outputs.shape[2] ** 0.5)
        # 4: Masked (causal) softmax to compute attention weights.
        # diag_val = torch.ones_like(outputs[0])
        # tril = torch.tril(diag_val)
        masks = mask[None, :, :].repeat(outputs.shape[0], 1, 1)  # [h*N, T, T2]
        padding = torch.ones_like(masks) * (-(2**32) + 1)
        outputs = torch.where(masks == 0, padding, outputs)
        outputs = F.softmax(outputs, dim=2)
        self.attn_wts_all = outputs  # [h*N, T, T2]
